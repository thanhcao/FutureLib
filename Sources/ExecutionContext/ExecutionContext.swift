...
public struct ConcurrentAsync: ExecutionContext {
     ...
     */
    public func execute(_ f: @escaping () -> ()) {
        DispatchQueue.global(qos: DispatchQoS.QoSClass.userInteractive).async(execute: f)
    }
}
...
public struct ConcurrentSync: ExecutionContext {
     ...
     */
    public func execute(_ f: () -> ()) {
        DispatchQueue.global(qos: DispatchQoS.QoSClass.userInteractive).sync(execute: f)
    }
}
